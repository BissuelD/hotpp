...
__all__ = ["ChebyshevPoly", "SpinChebyshevPoly", "BesselPoly", "MLPPoly"]
...
class ChebyshevPoly(RadialLayer):
    ...
    def radial(
        ...
        x = torch.clamp(
            (d - self.r_min) / (self.r_max - self.r_min),
            min=1e-6,
            max=1.0 - 1e-6,
        )
        out = torch.cos(torch.arccos(2 * x - 1) * self.n)
        # x = 2 * (1 - x) ** 2 - 1
    ...
    def replicate(self):
        return self.__class__(
            self.r_max, self.r_min, self.n_max, self.lamb, self.cutoff_fn
        )
...
class MLPPoly(RadialLayer):
    ...
    def replicate(self):
        return self.__class__(
            self.n_hidden, self.radial_fn.replicate(), self.activate_fn, self.cutoff_fn
        )

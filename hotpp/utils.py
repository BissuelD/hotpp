...
from typing import Literal, Optional, Dict, List, Callable, Tuple, Union
...
def expand_to(t: torch.Tensor, n_dim: int, dim: int = -1) -> torch.Tensor:
...
def multi_outer_product(v: torch.Tensor, n: int) -> torch.Tensor:
...
def find_distances(
    ...
    if 'rij' not in batch_data:
        ...
        if 'offset' in batch_data:
            batch_data['rij'] = (
                batch_data['coordinate'][idx_j]
                + batch_data['offset']
                - batch_data['coordinate'][idx_i]
            )
        else:
            batch_data['rij'] = (
                batch_data['coordinate'][idx_j] - batch_data['coordinate'][idx_i]
            )
...
def find_spin(
    ...
    if 'sij' not in batch_data:
        batch_data['sij'] = torch.sum(
            batch_data['si'][batch_data['idx_i']]
            * batch_data['si'][batch_data['idx_j']],
            dim=1,
        )
    return batch_data['mi'], batch_data['si'], batch_data['sij']


def find_moment(batch_data: Dict[str, torch.Tensor], n_way: int) -> torch.Tensor:
    if 'moment' + str(n_way) not in batch_data:
        find_distances(batch_data)
        batch_data['moment' + str(n_way)] = multi_outer_product(
            batch_data['uij'], n_way
        )
    return batch_data['moment' + str(n_way)]


def find_spin_moment(batch_data: Dict[str, torch.Tensor], n_way: int) -> torch.Tensor:
    if 'spin_moment' + str(n_way) not in batch_data:
        batch_data['spin_moment' + str(n_way)] = multi_outer_product(
            batch_data['si'], n_way
        )
...
def _scatter_mean(
    ...
    tmp = torch.zeros(shape, dtype=x.dtype, device=x.device)
    y = tmp.index_add(dim, idx_i, x)
    count = torch.zeros_like(tmp).index_add(dim, idx_i, torch.ones_like(x)).clamp(min=1)
    return y / count
...
def _aggregate(
    ...
    filter_tensor = moment_tensor.unsqueeze(1) * expand_to(
        fn, n_dim=r_way + 2
        ...
        for way1 in range(max_way1 + 1):
            for way2 in range(max_way2 + 1):
                for way3 in range(abs(way2 - way1), min(max_way3, way1 + way2) + 1, 2):
                    cls.oplist[(way1, way2, way3)] = aggregate_tensors_fn(
                        way1, way2, way3
                    )

...
class AtomicModule(nn.Module):
    ...
    def forward(
        ...
        if 'dipole' in output_tensors:
            batch_data['dipole_p'] = _scatter_add(
                output_tensors['dipole'], batch_data['batch']
            )
        if 'polar_diag' in output_tensors:
            polar_diag = _scatter_add(output_tensors['polar_diag'], batch_data['batch'])
            polar_off_diagonal = _scatter_add(
                output_tensors['polar_off_diagonal'], batch_data['batch']
            )
        ...
        if 'peratom_tensor_diag' in output_tensors:
            ...
            polar[:, [0, 1, 2], [0, 1, 2]] += output_tensors['peratom_tensor_diag'][
                :, None
            ]
            batch_data['peratom_tensor_p'] = polar
        if 'l3_tensor_diag' in output_tensors:
            polar_diag = _scatter_add(
                output_tensors['l3_tensor_diag'], batch_data['batch']
            )
            polar_off_diagonal = _scatter_add(
                output_tensors['l3_tensor_offdiag'], batch_data['batch']
            )
            polar = (
                expand_to(polar_diag, 4, -1)
                * expand_to(torch.eye(3, device=polar_diag.device), 4, 0)
                + polar_off_diagonal
            )
            polar = (
                polar
...
class MultiAtomicModule(AtomicModule):
    def __init__(self, models: Dict[str, AtomicModule]) -> None:
        ...
        for name, model in models.items():
            ...
            assert (
                self.target_way == model.target_way
            ), f"{name} target way {model.target_way} is different from {self.target_way}"
    ...
    def calculate(
        ...
        if 'site_energy' in self.target_way:
            output_tensors['site_energy'] = torch.zeros(
                (n_atoms), dtype=batch_data['coordinate'].dtype, device=device
            )
        if 'direct_forces' in self.target_way:
            output_tensors['direct_forces'] = torch.zeros(
                (n_atoms, n_dim), dtype=batch_data['coordinate'].dtype, device=device
            )
        if 'dipole' in self.target_way:
            output_tensors['dipole'] = torch.zeros(
                (n_atoms, n_dim), dtype=batch_data['coordinate'].dtype, device=device
            )
        if 'polar_diag' in self.target_way:
            output_tensors['polar_diag'] = torch.zeros(
                (n_atoms), dtype=batch_data['coordinate'].dtype, device=device
            )
            output_tensors['polar_off_diagonal'] = torch.zeros(
                (n_atoms, n_dim, n_dim),
                dtype=batch_data['coordinate'].dtype,
                device=device,
            )
        if 'peratom_tensor_diag' in self.target_way:
            output_tensors['peratom_tensor_diag'] = torch.zeros(
                (n_atoms), dtype=batch_data['coordinate'].dtype, device=device
            )
            output_tensors['peratom_tensor_offdiag'] = torch.zeros(
                (n_atoms, n_dim, n_dim),
                dtype=batch_data['coordinate'].dtype,
                device=device,
            )
        if 'l3_tensor_diag' in self.target_way:
            output_tensors['l3_tensor_diag'] = torch.zeros(
                (n_atoms, n_dim), dtype=batch_data['coordinate'].dtype, device=device
            )
            output_tensors['l3_tensor_offdiag'] = torch.zeros(
                (n_atoms, n_dim, n_dim, n_dim),
                dtype=batch_data['coordinate'].dtype,
                device=device,
            )
        if 'peratom_l3_tensor_diag' in self.target_way:
            output_tensors['peratom_l3_tensor_diag'] = torch.zeros(
                (n_atoms, n_dim), dtype=batch_data['coordinate'].dtype, device=device
            )
            output_tensors['peratom_l3_tensor_offdiag'] = torch.zeros(
                (n_atoms, n_dim, n_dim, n_dim),
                dtype=batch_data['coordinate'].dtype,
                device=device,
            )
